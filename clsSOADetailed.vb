...
Public Class clsSOADetailed
    ...
    Public Function ReportTrans() As Boolean
        ...
        lsSQL = " SELECT " &
                    ...
                    " , f.sCompnyNm sCompnyNm " &
                    " , a.sSourceCd sSourceCd " &
                    " , a.cTranStat cTranStat " &
                    " , a.sRemarksx sRemarksx " &
                    " , a.nTranTotl nTranTotl " &
                    " , d.sTransNox dsTransNox " &
                    " , d.dTransact dsTransact " &
                    " , e.sORNumber sORNumber " &
                    " , c.dBilledxx dBilledxx " &
                    " , c.cPaidxxxx cPaidxxxx " &
                    " , c.dPaidxxxx dPaidxxxx " &
                    " , c.nAmountxx nAmountxx " &
                    " , b.nEntryNox nEntryNox " &
                    "  FROM Billing_Master a " &
                    " INNER JOIN Billing_Detail b " &
                    " ON a.sTransNox = b.sTransNox " &
                    " LEFT JOIN Delivery_Service_Trans c " &
                    " ON b.sSourceNo = c.sTransNox " &
                    " LEFT JOIN SO_Master d " &
                    " ON c.sSourceNo = d.sTransNox " &
                    " LEFT JOIN Receipt_Master e " &
                    " ON d.sTransNox = e.sSourceNo " &
                    " LEFT JOIN Client_Master f " &
                    " ON a.sClientID = f.sClientID " &
                    " LEFT JOIN Delivery_Service g " &
                    " ON c.sRiderIDx = g.sRiderIDx " &
        ...
        If Not clsRpt.initReport("SOADet") Then
            Return False
        End If
        ...
        loTxtObj.Text = "Statement of Account Detailed Report"
    ...
    Private Function addRow(ByVal lnRow As Integer, ByVal foSchemaTable As DataTable) As DataRow
        ...
        'master's group field
        'loDtaRow.Item("nField01") =
        ...
        loDtaRow.Item("lField01") = p_oDTSrce(lnRow).Item("nTranTotl")

        'detail
        'loDtaRow.Item("nField01") = p_oDTSrce(lnRow).Item("nEntryNox")
        loDtaRow.Item("sField07") = p_oDTSrce(lnRow).Item("dsTransNox")
        loDtaRow.Item("sField08") = Format(p_oDTSrce(lnRow).Item("dsTransact"), "MMMM dd, yyyy")
        loDtaRow.Item("sField09") = p_oDTSrce(lnRow).Item("sORNumber")
        loDtaRow.Item("sField10") = Format(p_oDTSrce(lnRow).Item("dBilledxx"), "MMMM dd, yyyy")
        loDtaRow.Item("sField11") = DetailTranStatus(p_oDTSrce(lnRow).Item("cPaidxxxx"))
        loDtaRow.Item("sField12") = Format(p_oDTSrce(lnRow).Item("dPaidxxxx"), "MMMM dd, yyyy")
        loDtaRow.Item("lField02") = p_oDTSrce(lnRow).Item("nAmountxx")
